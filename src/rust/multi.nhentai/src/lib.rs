...
fn get_manga_list(filters: Vec<Filter>, page: i32) -> Result<MangaPageResult> {
	...
	if is_sauce_code {
		...
		let mut url = String::from("https://nhentai.to/api/galleries/search?query=");
...
fn get_manga_details(id: String) -> Result<Manga> {
	...
	let mut url = String::from("https://nhentai.to/g/");
	url.push_str(&id);
...
fn get_page_list(_: String, id: String) -> Result<Vec<Page>> {
	...
	for (i, page) in pages_arr.enumerate() {
		...
		let mut url = String::from("https://i.nhentai.to/galleries/");
...
pub fn handle_url(url: String) -> Result<DeepLink> {
	let url = &url[20..]; // remove "https://nhentai.to/"
